...
`unwrap()` returns the output or raises `RuntimeExecutionError`; `without_output()` returns an audit copy for attaching as `JobSignalExtractorOutput.execution` (provenance without nesting the full typed output).
